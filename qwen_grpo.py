...
def length_penalty(gen_len, max_new_tokens, threshold_frac=0.8):
    """Flat penalty applied when generation exceeds threshold_frac of the budget."""
...
def collect_rollouts(
    ...
    """Sample rollouts and cache old_logp / ref_logp / rewards. Runs once per outer step.

    `old_logp` is computed via the same fresh full-sequence forward path that
    `new_logp` will use in `compute_grpo_loss`, so the GRPO ratio is exactly 1.0
    at the first inner epoch (modulo true fp32 noise). Costs one extra forward per
    rollout vs capturing during sampling, but eliminates the cached-vs-uncached
    numerical bias that would otherwise corrupt the gradient estimator.
    """
    ...
    for _ in range(num_rollouts):
        ...
        with torch.no_grad():
            old_logp, entropy = sequence_logprob_and_entropy(
                model, token_ids, prompt_len, return_entropy=True
            )
            if kl_coeff:
        ...
        len_penalty = length_penalty_weight * length_penalty(
            gen_len, max_new_tokens, length_penalty_threshold_frac
        )
        reward = correctness + format_bonus - len_penalty

        rollouts.append({
            "token_ids": token_ids,
            "prompt_len": prompt_len,
            "old_logp": old_logp.detach(),
            "ref_logp": ref_logp.detach() if ref_logp is not None else None,
            "reward": reward,
...
def compute_grpo_loss(
    ...
    """Recompute new_logp under the (possibly updated) model and form the loss.

    Runs `inner_epochs` times per outer step. Only this call needs to be repeated —
    rewards, advantages, old_logp, and ref_logp are all stable across inner epochs.
    """
    ...
    new_logps = torch.stack([
        sequence_logprob_and_entropy(model, r["token_ids"], r["prompt_len"])
        for r in rollouts
    ])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of inner update iterations per step. Use 1 for on-policy.",
    )
    parser.add_argument(
        "--kl_coeff",
        type=float,
        default=0.02,
        help="KL penalty coefficient. Author's stable run on this codebase uses 0.02.",
    )
    parser.add_argument(
        "--seed",
        type=str,
    ...
    parser.add_argument(
        ...
        default=0.05,
        help=(
            "Tiny additive bonus when the rollout is correct AND has \\boxed{}. "
            "Mathematically inert under advantage normalization since "
            "correctness already implies has_box; kept for readability."
        ),
    )
    parser.add_argument(
        ...
        default=0.1,
        help=(
            "Flat penalty applied when gen_len exceeds threshold_frac * "
            "max_new_tokens. Discourages saturation regardless of correctness."
        ),
    )
    parser.add_argument(
        "--length_penalty_threshold_frac",
        type=float,
        default=0.8,
