# Adapted from Sebastian Raschka's "Build a Reasoning Model (From Scratch)"
# (Apache License 2.0). Modified to call DeepSeek's API directly instead
# of OpenRouter.
...
# DeepSeek's API is OpenAI-compatible. Base URL accepts both
# https://api.deepseek.com and https://api.deepseek.com/v1.
DEEPSEEK_CHAT_URL = "https://api.deepseek.com/v1/chat/completions"

# Legacy aliases that reject sampling params and always run in thinking mode.
# Scheduled for deprecation 2026-07-24; kept for back-compat.
LEGACY_REASONER_MODELS = {"deepseek-reasoner"}
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            "DeepSeek model ID. Current options: 'deepseek-v4-pro' "
            "(1M context, up to 384K output), 'deepseek-v4-flash'. Legacy "
            "aliases 'deepseek-chat' and 'deepseek-reasoner' are deprecated "
            "on 2026-07-24."
        ),
    )
    parser.add_argument(
        ...
        help=(
            "Toggle thinking mode for V4 models. Sent as the top-level "
            "'thinking' field in the request body. Enabled returns a "
            "reasoning_content trace alongside the final content."
        ),
    )
    parser.add_argument(
        "--reasoning_effort",
        ...
        help=(
            "Optional reasoning_effort hint for thinking-mode generations. "
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Sampling temperature (ignored by legacy deepseek-reasoner)",
    )
    parser.add_argument(
        "--top_p",
        type=float,
        default=1.0,
        help="Top-p sampling parameter (ignored by legacy deepseek-reasoner)",
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Number of parallel DeepSeek requests. "
...
def parse_deepseek_response(decoded):
    ...
    if not isinstance(first_choice, dict):
        raise RuntimeError("DeepSeek response has invalid choices format.")
    ...
    # DeepSeek's reasoner exposes its chain-of-thought via
    # message.reasoning_content (OpenAI-compatible field name).
    thinking = ""
    if isinstance(message, dict):
        if "reasoning_content" in message:
            thinking = content_to_text(message["reasoning_content"])
        if not thinking and "reasoning" in message:
            thinking = content_to_text(message["reasoning"])
        if not thinking and "thinking" in message:
            thinking = content_to_text(message["thinking"])
    ...
    if not content:
        ...
        raise RuntimeError(
            "DeepSeek response did not contain parseable assistant content. "
...
def query_deepseek_chat(
    ...
    is_legacy_reasoner = model in LEGACY_REASONER_MODELS
    if not is_legacy_reasoner:
        payload["temperature"] = temperature
        payload["top_p"] = top_p
        # V4 models accept an explicit thinking-mode toggle (default enabled).
        # Sent at the top level here because we hit the raw HTTP endpoint —
        # OpenAI-SDK callers would route this through extra_body.
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            return parse_deepseek_response(decoded)

        except error.HTTPError as exc:
            err_body = exc.read().decode("utf-8", errors="replace")
            last_error = RuntimeError(
                f"HTTP {exc.code} from DeepSeek at {DEEPSEEK_CHAT_URL}: {err_body}"
            )
        except (
            error.URLError,
            TimeoutError,
    ...
    raise RuntimeError(
        f"Failed to query DeepSeek after {max_retries} attempt(s). "
...
if __name__ == "__main__":
    ...
    print(f"Using DeepSeek API: {DEEPSEEK_CHAT_URL}")
