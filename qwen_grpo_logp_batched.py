"""GRPO trainer with sequential sampling + batched logprob scoring.

This is qwen_grpo.py with one targeted change: the logprob computation for
old_logp / ref_logp / new_logp uses a single batched forward over the rollouts
instead of N sequential forwards. Sampling stays sequential (batch=1 per
rollout) — that's the path that produced 50% MATH-500 in the unbatched run, and
batching it introduced kernel-level numerical drift that biased the rollout
distribution.

Why this is safe:
- old_logp and new_logp both use the same batched-N forward path on the same
  model snapshot. With inner_epochs=1, ratio = exp(new - old) is exactly 1.0
  to fp32 precision because the bias cancels.
- ref_logp is on a different model, so the batched-bias profiles don't
  cancel exactly in the KL term. The residual is bounded by the per-token
  logit drift (~0.09 mean) times kl_coeff (0.02) — vanishingly small impact.
- Reward signal depends only on text → unchanged.

What this buys:
- ~5-10% wall-clock per step (sampling still dominates).
- Lets us bump num_rollouts cheaply since one batched logp forward scales
  much better than N sequential forwards at small batches.
"""
import argparse
import copy
import time
...
def sample_response(
    ...
    """Sequential autoregressive sampling at batch=1. Unchanged from qwen_grpo.py."""
...
def batched_sequence_logprob(model, token_ids_list, prompt_lens, return_entropy=False, pad_id=0):
    """Sum-of-logprobs (and optional mean entropy) for each rollout via one padded forward.

    Causal attention means real positions never attend to padding, so per-row outputs
    at real positions are equivalent to running each sequence solo (modulo fp32
    reduction order, which we accept since the bias cancels in PPO ratios).
    """
...
def collect_rollouts(
    ...
    # Sequential sampling — preserves the proven rollout distribution.
    sampled = []
...
def compute_grpo_loss(
    ...
    # Single batched with-grad forward for new_logp.
...
def append_step_metrics(
    ...
    policy_ratio_str = (
        "" if policy_ratio is None else f" policy_ratio={policy_ratio:.4f}"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        description="GRPO trainer with sequential sampling + batched logp scoring.",
    )
    ...
    parser.add_argument("--eval_on_checkpoint", type=int, default=0)
    parser.add_argument("--skip-zero-advantage-updates", action="store_true")
    parser.add_argument("--show_eta", action="store_true")
